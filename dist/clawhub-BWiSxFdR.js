...
import { a as fetchClawHubPackageVersion, d as satisfiesPluginApiRange, i as fetchClawHubPackageDetail, l as resolveLatestVersionFromPackage, n as downloadClawHubPackageArchive, s as parseClawHubPluginSpec, t as ClawHubRequestError, u as satisfiesGatewayMinimum } from "./clawhub-C7cd_hWw.js";
import { i as installPluginFromArchive } from "./installs-Bpfupz8S.js";
import path from "node:path";
import fs from "node:fs/promises";
//#region src/plugins/clawhub.ts
const OPENCLAW_PLUGIN_API_VERSION = "1.2.0";
...
function validateClawHubPluginPackage(params) {
	...
	if (compatibility?.pluginApiRange && !satisfiesPluginApiRange("1.2.0", compatibility.pluginApiRange)) return buildClawHubInstallFailure(`Plugin "${pkg.name}" requires plugin API ${compatibility.pluginApiRange}, but this OpenClaw runtime exposes ${OPENCLAW_PLUGIN_API_VERSION}.`, CLAWHUB_INSTALL_ERROR_CODE.INCOMPATIBLE_PLUGIN_API);
...
function logClawHubPackageSummary(params) {
	...
	const compatibilityParts = [pkg.compatibility?.pluginApiRange ? `pluginApi=${pkg.compatibility.pluginApiRange}` : null, pkg.compatibility?.minGatewayVersion ? `minGateway=${pkg.compatibility.minGatewayVersion}` : null].filter(Boolean);
