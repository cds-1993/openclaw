...
if (!isMainModule({
	...
	const { installGaxiosFetchCompat } = await import("./gaxios-fetch-compat-CqonwvEB.js");
	...
	function tryHandleRootVersionFastPath(argv) {
		...
		Promise.all([import("./version-CCtzb5bS.js"), import("./git-commit-COAqfbAI.js")]).then(([{ VERSION }, { resolveCommitHash }]) => {
...
function tryHandleRootHelpFastPath(argv, deps = {}) {
	...
	import("./root-help-CCoVwqz0.js").then(({ outputRootHelp }) => {
		outputRootHelp();
	}).catch(handleError);
	return true;
}
function runMainOrRootHelp(argv) {
	...
	import("./run-main-CCt_Uyy-.js").then(({ runCli }) => runCli(argv)).catch((error) => {
