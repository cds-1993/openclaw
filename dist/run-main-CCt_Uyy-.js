...
import { t as emitCliBanner } from "./banner-Be9UXpWc.js";
...
const routeHealth = {
	...
	run: async (argv) => {
		...
		const { healthCommand } = await import("./health-gy-pCN-a.js");
		await healthCommand({
			json,
			timeoutMs,
			verbose
...
const routeStatus = {
	...
	run: async (argv) => {
		...
		if (timeoutMs === null) return false;
		if (json) {
			const { statusJsonCommand } = await import("./status-json-DvE11IAb.js");
		...
		const { statusCommand } = await import("./status-BdVQ1yEt.js");
		await statusCommand({
			json,
			deep,
			all,
...
const routeGatewayStatus = {
	...
	run: async (argv) => {
		...
		const { runDaemonStatus } = await import("./status-CloC3sMW.js");
...
const routeSessions = {
	...
	run: async (argv) => {
		...
		const { sessionsCommand } = await import("./sessions-0S9SFskv.js");
...
const routeAgentsList = {
	...
	run: async (argv) => {
		...
		const { agentsListCommand } = await import("./agents-CX5pekvf.js");
...
const routes = [
	...
	routeMemoryStatus,
	{
		...
		run: async (argv) => {
			...
			const json = hasFlag(argv, "--json");
			const { runConfigGet } = await import("./config-cli-CSOykbJZ.js");
			await runConfigGet({
				path: pathArg,
				json
			});
	...
	},
	{
		...
		run: async (argv) => {
			...
			const pathArg = positionals[0];
			if (!pathArg) return false;
			const { runConfigUnset } = await import("./config-cli-CSOykbJZ.js");
	...
	},
	{
		...
		run: async (argv) => {
			...
			const { modelsListCommand } = await import("./models-C3QGNGda.js");
	...
	},
	{
		...
		run: async (argv) => {
			...
			const { modelsStatusCommand } = await import("./models-C3QGNGda.js");
...
async function prepareRoutedCommand(params) {
	...
	const { ensureConfigReady } = await import("./config-guard-DEoSs1yY.js");
	...
	if (typeof params.loadPlugins === "function" ? params.loadPlugins(params.argv) : params.loadPlugins) {
		const { ensurePluginRegistryLoaded } = await import("./plugin-registry-BEYmhNwC.js");
...
async function closeCliMemoryManagers() {
	try {
		const { closeAllMemorySearchManagers } = await import("./search-manager-CDSANhs-.js");
...
async function runCli(argv = process$1.argv) {
	...
	try {
		if (shouldUseRootHelpFastPath(normalizedArgv)) {
			const { outputRootHelp } = await import("./root-help-CCoVwqz0.js");
			outputRootHelp();
			return;
		}
		if (await tryRouteCli(normalizedArgv)) return;
		enableConsoleCapture();
		const { buildProgram } = await import("./program-DEyDeACK.js");
		...
		if (primary) {
			const { getProgramContext } = await import("./program-context-C9981WD2.js");
			const ctx = getProgramContext(program);
			if (ctx) {
				const { registerCoreCliByName } = await import("./command-registry-B2CqWLUg.js");
				await registerCoreCliByName(program, ctx, primary, parseArgv);
			}
			const { registerSubCliByName } = await import("./register.subclis-RpOj6cyO.js");
		...
		if (!shouldSkipPluginCommandRegistration({
			...
			const { loadValidatedConfigForPluginRegistration } = await import("./register.subclis-RpOj6cyO.js");
