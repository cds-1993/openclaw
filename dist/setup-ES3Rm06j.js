...
import { n as buildPluginCompatibilityNotices, o as formatPluginCompatibilityNotice } from "./status-DlXfYTpa.js";
...
async function runSetupWizard(opts, runtime = defaultRuntime, prompter) {
	const onboardHelpers = await import("./onboard-helpers-CunjcgA4.js");
	...
	if (mode === "remote") {
		const { promptRemoteGatewayConfig } = await import("./onboard-remote-DZPeHr2g.js");
		const { logConfigUpdated } = await import("./logging-CuejgkN3.js");
	...
	const { applyLocalSetupWorkspaceConfig } = await import("./onboard-config-CdK_PIV7.js");
	...
	const { promptAuthChoiceGrouped } = await import("./auth-choice-prompt-DMViSca0.js");
	const { promptCustomApiConfig } = await import("./onboard-custom-Bcs4Aq7k.js");
	const { applyAuthChoice, resolvePreferredProviderForAuthChoice, warnIfModelConfigLooksOff } = await import("./auth-choice-C0NNpFf9.js");
	const { applyPrimaryModel, promptDefaultModel } = await import("./model-picker-CNvnryra.js");
	...
	const { configureGatewayForSetup } = await import("./setup.gateway-config-D-pCmRyI.js");
	...
	if (opts.skipChannels ?? opts.skipProviders) await prompter.note("Skipping channel setup.", "Channels");
		...
		const { setupChannels } = await import("./onboard-channels-CE7rQgOV.js");
	...
	if (opts.skipSearch) await prompter.note("Skipping search setup.", "Search");
		...
		const { setupSearch } = await import("./onboard-search-C8jdFq7J.js");
	...
	if (opts.skipSkills) await prompter.note("Skipping skills setup.", "Skills");
		...
		const { setupSkills } = await import("./onboard-skills-BmeNXJM0.js");
		nextConfig = await setupSkills(nextConfig, workspaceDir, runtime, prompter);
	}
	const { setupInternalHooks } = await import("./onboard-hooks-BCpXfLB1.js");
	...
	await writeConfigFile(nextConfig);
	const { finalizeSetupWizard } = await import("./setup.finalize-DxTEeNCc.js");
